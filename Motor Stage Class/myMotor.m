classdef myMotor < handle 
    ...
    % Connect your BSCxxx, BBD30X, KDC101 (PRM1Z8 / PRM1TZ8 / KS1-Z8 ... devices with Z8 motor) or K10CR1
    % translation/rotation stage(s) to the PC USB port
    %
    % Example for K10CR1 / KCD101 rotational stage:
    % mlist=motor.listdevices % List connected devices
    % mot = motor             % Create a motor object  
    % mot.connect(mlist{1})   % Connect the first device in the list of devices
    ...
    %
    % Example for BSC302 controller
    % motXYZ = motor;          % create a motor object
    % motXYZ.connect('70xxx')  % connect to a controller with a serial number 70xxx
    % motXYZ.enable(1)         % enable channel 1 if not enabled
    % motXYZ.enable(2)         % enable channel 2 if not enabled
    % motXYZ.disable(3)        % disable channel 3 if enabled
    % motXYZ.ishomed           % check if channels are homed
    % motXYZ.home(1)           % perform homing channel 1
    % motXYZ.home(2)           % perform homing channel 2
    % motXYZ.position          % get the current position for all channels
    % motXYZ.moveto([15,25,0])   % move to position 15 mm on Ch1, 25 mm on Ch2, 0 on Ch3 (array of 3 values, 1 per channel independent of enable or disable) 
    % motXYZ.maxvelocity = [10,50,50];   % set maxvelocity for movement to 10 mm/s for Ch1, 50 mm/s for Ch2 and 50 mm/s for Ch3
    % motXYZ.acceleration = [50,50,50];% set acceleration to 50 mm/s2 for both channels
    % motXYZ.moveto([55,37.5,0]) % move to position [55 mm, 37.5,0]
    % motXYZ.disconnect        % disconnect the device
    % clear motXYZ             % Clear device object from memory
    %
    % based on a code of Andriy Chmyrov 
    ...
    %
    % Version History:
    % 3.0 2022-10-14 - added support of BSC motion controllers
    % 
    % F.Oliveira, Last Update, 2023-05-22 - add improve new function
    % 'moveToNoWaitTime', wait time added before movement in case the motor
    % still moving from the previous action.
    
    
    properties (Constant, Hidden)
       % path to DLL files (edit as appropriate)
       KINESISPATHDEFAULT_64BITS = 'C:\Program Files\Thorlabs\Kinesis\'
       KINESISPATHDEFAULT_32BITS = 'C:\Program Files (x86)\Thorlabs\Kinesis\';
    ...
    properties (Hidden)
       ...
       channel;                     % channels for multi-channel controller
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods
        ...
        % =================================================================
        function h = myMotor() % Constructor - Instantiate motor object
            myMotor.loaddlls; % Load DLLs (if not already loaded)
        end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    methods (Sealed)
        ...
        function connect(h,serialNo)  % Connect device
            ...
            if ~h.initialized
                ...
                switch(h.prefix)
                    ...
                    case Thorlabs.MotionControl.Benchtop.StepperMotorCLI.BenchtopStepperMotor.DevicePrefix70
                        % 70 - Serial number corresponds to BSC70X type devices 
                    ...
                    case Thorlabs.MotionControl.Benchtop.StepperMotorCLI.BenchtopStepperMotor.DevicePrefix40
                        % 40 - Serial number corresponds to BSC40X type devices 
                        h.deviceNET = Thorlabs.MotionControl.Benchtop.StepperMotorCLI.BenchtopStepperMotor.CreateBenchtopStepperMotor(serialNo);
            ...
            if h.isconnected
                try
                    switch(h.prefix)
                        ...
                        case {Thorlabs.MotionControl.Benchtop.BrushlessMotorCLI.BenchtopBrushlessMotor.DevicePrefix103,...
                                Thorlabs.MotionControl.Benchtop.StepperMotorCLI.BenchtopStepperMotor.DevicePrefix70}
        ...
        % =================================================================
        % FUNCTION: MOVE TO WITHOUT WAIT TIME
        % Function equivalent to 'moveto' but without wait time. 
        function moveToNoWaitTime(h,position)
            switch(h.prefix)
                case {Thorlabs.MotionControl.KCube.DCServoCLI.KCubeDCServo.DevicePrefix,...                 % Thorlabs K-Cube DC servo motor.
                      Thorlabs.MotionControl.IntegratedStepperMotorsCLI.CageRotator.DevicePrefix,...        % Thorlabs K10CR1.
                      Thorlabs.MotionControl.Benchtop.StepperMotorCLI.BenchtopStepperMotor.DevicePrefix40}  % Thorlabs Benchtop Stepper motor controller 1 channel.
                    try
                        h.deviceNET.Wait(h.TIMEOUTMOVE)              % Wait for move to finish
                        workDone=h.deviceNET.InitializeWaitHandler(); % Initialise Waithandler for timeout
                        h.deviceNET.MoveTo(position, workDone);       % Move devce to position via .NET interface
%                         h.deviceNET.Wait(h.TIMEOUTMOVE);              % Wait for move to finish
                ...
                case {Thorlabs.MotionControl.Benchtop.BrushlessMotorCLI.BenchtopBrushlessMotor.DevicePrefix103,...  % Thorlabs Benchtop Brushless motor controller multiple channels.
                        Thorlabs.MotionControl.Benchtop.StepperMotorCLI.BenchtopStepperMotor.DevicePrefix70}        % Thorlabs Benchtop Stepper motor controller multiple channels.
                    if length(position) ~= h.deviceInfoNET.NumChannels
                        error([int2str(h.deviceInfoNET.NumChannels) ' coordinates expected for the device ',h.controllername,' with serial number ',h.serialnumber,'!']);
                    end
                    for km = 1:double(h.deviceInfoNET.NumChannels)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    methods (Static)
        ...
        function serialNumbers = listdevices()  % Read a list of serial number of connected devices
            ...
            nPref = NET.createArray('System.Int32',5);   % 3 --> 5
            ...
            nPref(4) = Thorlabs.MotionControl.Benchtop.StepperMotorCLI.BenchtopStepperMotor.DevicePrefix70;
        ...
        function loaddlls() % Load DLLs
            ...
            if exist(myMotor.KINESISPATHDEFAULT_64BITS) == 7        % Default Kinesis path, 64-bits version, Microsoft Windows 64-bits. 
                path = myMotor.KINESISPATHDEFAULT_64BITS;
            elseif exist(myMotor.KINESISPATHDEFAULT_32BITS) == 7    % Default Kinesis path, 32-bits version, Microsoft Windows 32-bits.
                path = myMotor.KINESISPATHDEFAULT_32BITS;
            else                                            % Matlab path currently in use.
                path = '';
            end
            
            if ~exist(myMotor.INTEGSTEPCLASSNAME,'class')
                try   % Load in DLLs if not already loaded
                    NET.addAssembly([path,myMotor.DEVICEMANAGERDLL]);
                    NET.addAssembly([path,myMotor.GENERICMOTORDLL]);
                    NET.addAssembly([path,myMotor.DCSERVODLL]); 
                    NET.addAssembly([path,myMotor.INTEGSTEPDLL]); 
                    NET.addAssembly([path,myMotor.BRUSHLESSDLL]);
                    NET.addAssembly([path,myMotor.STEPPERDLL]); 
                catch % DLLs did not load
                    msg = ["Thorlabs Kinesis was not found in its default path."...
                           "Unable to load .NET assemblies"];
                    title = 'Error';
                    icon = 'error';
